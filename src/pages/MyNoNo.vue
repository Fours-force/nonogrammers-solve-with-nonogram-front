<template>
	<div class="w-6/12 mt-2 max:w-8/12 font-brr min-w-min">
		<h1
			class="pb-5 pl-5 text-3xl border-b-2 mb-7 font-brr text-nav-navy"
			th:text="${title}"
		></h1>
...
async function getUserNoNo() {
	await axios
		.get("http://localhost:8089/nono/" + currentRouter.params.id + "/" + currentRouter.params.nonoId)
		.then((response) => {
			const data = response.data;
			console.log("🚀 ~ file: MyNoNo.vue:94 ~ .then ~ data:", data)
